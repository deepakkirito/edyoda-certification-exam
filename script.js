$(document).ready(function () {
    ...
    let teamsDefault = [
            ...
            "Players": [
                {
                    ...
                    "profile": "https://www.kkr.in/static-assets/images/players/2020/63961.png?v=4.10&w=200",
                ...
                },
                {
                    ...
                    "profile": "https://www.kkr.in/static-assets/images/players/2020/63649.png?v=4.10&w=200",
                    "from": "Kolkata Knight Riders",
                    "price": "8 crore",
                    "isPlaying": true,
                    "description": "Batsmen"
        ...
        },
        {
            ...
            "profile": "https://logos-world.net/wp-content/uploads/2022/05/RCB-Symbol.png",
            "Team_Short_Code": "RCB"
        }
        // ,
        // {
    ...
    let displayTeams = function () {
        ...
        for (let i = 0; i < teams.length; i++) {
            container.append('<img title=' + teams[i].Team_Short_Code + ' class="card" src=' + teams[i].profile + '>')
        }
    }
    displayTeams();
    ...
    searchIcon.on("click", function (e) {
        ...
        for (let i = 0; i < teams.length; i++) {
            if (searchValue[0].value == '') {
                ...
                teamWrapper.hide();
            } else if (teams[i].Team_Short_Code.toLowerCase() == searchValue[0].value.toLowerCase() || teams[i].Team_Name.toLowerCase() == searchValue[0].value.toLowerCase()) {
                teamWrapper.fadeIn();
                teamContainer.html('')
                counter++;
                container.append('<img title=' + teams[i].Team_Short_Code + ' class="card" src=' + teams[i].profile + '>')
                totalTeams.text('Search Results for ' + searchValue[0].value + ' : ' + counter)
                for (let j = 0; j < teams[i].Players.length; j++) {
                    teamContainer.append('<div id=P' + teams[i].Players[j].id + ' class="playerBox"><img class=' + teams[i].Team_Short_Code + ' id=' + teams[i].Players[j].id + ' src=' + teams[i].Players[j].profile + '><div id="playerDetails" class="playerDetails" style= background:' + teams[i].bgcolor + ' ><h5>' + teams[i].Players[j].playerName + '</h5><p>' + teams[i].Players[j].description + '</p><p>Price : ' + teams[i].Players[j].price + '</p><p>Team : ' + teams[i].Players[j].from + '</p><p>Playing : ' + teams[i].Players[j].isPlaying + '</p></div><div id="playerDetailsClick" class="playerDetailsClick" style= background:' + teams[i].bgcolor + ' ><h5>' + teams[i].Players[j].playerName + '</h5><p>' + teams[i].Players[j].description + '</p><p>Price : ' + teams[i].Players[j].price + '</p><p>Team : ' + teams[i].Players[j].from + '</p><p>Playing : ' + teams[i].Players[j].isPlaying + '</p></div></div>')
                }
            } else if (teams[i].Team_Short_Code.toLowerCase().includes(searchValue[0].value.toLowerCase()) || teams[i].Team_Name.toLowerCase().includes(searchValue[0].value.toLowerCase())) {
                counter++;
                container.append('<img title=' + teams[i].Team_Short_Code + ' class="card" src=' + teams[i].profile + '>')
                totalTeams.text('Search Results for ' + searchValue[0].value + ' : ' + counter)
                teamWrapper.hide();
    ...
    $('input').on('keyup', function (e) {
        teamPlayerInput = {
            ...teamPlayerInput, [e.target.id]: e.target.value
        }
    })

    $('#saveTeam').on("click", function (e) {
        console.log(teamPlayerInput, selectInput, Object.keys(teamPlayerInput).length);
        if (selectInput == 'team') {
            ...
            if (Object.keys(teamPlayerInput).length == 6) {
                for (let i = 0; i < teams.length; i++) {
                    if (teams[i].Team_Short_Code.toLowerCase() == teamPlayerInput.from.toLowerCase()) {
                        ...
                        if (teams[i].Players == undefined) {
                            ...
                            teams[i].Players[teams[i].Players.length].id = teams[i].Players.length;
                            teams[i].Players.push(teamPlayerInput)
    ...
    for (let i = 0; i < teams.length; i++) {
        if (teams[i].Team_Short_Code == window.localStorage.getItem('teamDisplay')) {
            ...
            if (teams[i].Team_Short_Code == e.target.className) {
                for (let j = 0; j < teams[i].Players.length; j++) {
                    if (teams[i].Players[j].id == e.target.id) {
                        $('.' + e.target.className).css({ 'border-radius': '5px 0px 0px 5px' });
                    } else {
    ...
    closePlayer.on("click", function () {
        closePlayer.hide();
        for (let i = 0; i < teams.length; i++) {
            if (teams[i].Team_Short_Code.toLowerCase() == searchValue[0].value.toLowerCase() || teams[i].Team_Name.toLowerCase() == searchValue[0].value.toLowerCase()) {
                teamContainer.html('')
                for (let j = 0; j < teams[i].Players.length; j++) {
                    teamContainer.append('<div id=P' + teams[i].Players[j].id + ' class="playerBox"><img class=' + teams[i].Team_Short_Code + ' id=' + teams[i].Players[j].id + ' src=' + teams[i].Players[j].profile + '><div id="playerDetails" class="playerDetails" style= background:' + teams[i].bgcolor + ' ><h5>' + teams[i].Players[j].playerName + '</h5><p>' + teams[i].Players[j].description + '</p><p>Price : ' + teams[i].Players[j].price + '</p><p>Team : ' + teams[i].Players[j].from + '</p><p>Playing : ' + teams[i].Players[j].isPlaying + '</p></div><div id="playerDetailsClick" class="playerDetailsClick" style= background:' + teams[i].bgcolor + ' ><h5>' + teams[i].Players[j].playerName + '</h5><p>' + teams[i].Players[j].description + '</p><p>Price : ' + teams[i].Players[j].price + '</p><p>Team : ' + teams[i].Players[j].from + '</p><p>Playing : ' + teams[i].Players[j].isPlaying + '</p></div></div>')
                }
